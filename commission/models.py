...
class DepositSummary(models.Model):
    ...
    div_1m = models.CharField(
        max_length=10, blank=True, default="", choices=DIV_CHOICES, verbose_name="1개월전분급"
    )
    div_2m = models.CharField(
        max_length=10, blank=True, default="", choices=DIV_CHOICES, verbose_name="2개월전분급"
    )
    div_3m = models.CharField(
        max_length=10, blank=True, default="", choices=DIV_CHOICES, verbose_name="3개월전분급"
    )
...
class DepositUploadLog(models.Model):
    """
    deposit_upload_log
    - 부서(part) + 업로드구분(upload_type)별 "마지막 업로드 일자" 표시용
...
# =============================================================================
# Approval/Efficiency (결재/효율) 도메인
# =============================================================================

class ApprovalExcelUploadLog(models.Model):
    """월도+부서+kind 기준 업로드 로그"""
    KIND_CHOICES = (
        ("efficiency", "지점효율"),
        ("approval", "수수료결재"),
    )
    ...
    def __str__(self) -> str:
        return f"{self.ym} / {self.part or '전체'} / {self.kind} / {self.file_name}"
...
class EfficiencyPayExcess(models.Model):
    """
    efficiency_pay_excess
    - 지점효율지급 초과현황(월도별)
    - 업로드(kind=efficiency)로 채워짐
    """
    ym = models.CharField(max_length=7, db_index=True, verbose_name="월도(YYYY-MM)")
